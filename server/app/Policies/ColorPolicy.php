...
class ColorPolicy
{
    ...
    public function create(User $user)
    {
        return (
            ...
            $user->hasPermission(UserPermissionsEnum::CreateProductColors))
            && $user->isActive()
    ...
    public function update(User $user, Color $color)
    {
        return (
            ($user->isAdministrator() ||
            $user->hasPermission(UserPermissionsEnum::EditProductColors))
            && $user->isActive()
        ) ? Response::allow() : Response::deny($this->message);
    }
    ...
    public function delete(User $user, Color $color)
    {
        return (
            ($user->isAdministrator() ||
            $user->hasPermission(UserPermissionsEnum::DeleteProductColors))
            && $user->isActive()
        ) ? Response::allow() : Response::deny($this->message);
    }
